...
class GroupLFQEmbedding(nn.Module):
    def __init__(self, K, D):
        super().__init__()
        K = int(K)
        D = int(D)
        num_codebooks = 2
        codebook_size = int(math.sqrt(K))
        self.vq_layer = LFQ(dim=D, codebook_size=codebook_size, num_codebooks=num_codebooks)
...
class VQStepWiseTransformer(nn.Module):
    def __init__(self, config, feature_dim):
        ...
        if "ma_update" in config and not (config.ma_update):
            ...
            self.codebook = GroupLFQEmbedding(config.K, config.trajectory_embd)
...
class TransformerPrior(nn.Module):
    ...
    def __init__(self, config):
        ...
        codebook_size = int(math.sqrt(config.K))
        self.tok_emb = MultiGroupEmbedding(codebook_size=codebook_size, n_embd=config.n_embd, n_group=2)
        ...
        self.head = MultiGroupLL(config.n_embd, codebook_size, n_group=2)
        # weight tying
        self.wt = True
        if self.wt:
            # self.tok_emb.weight = self.head.weight
            for emb, head in zip(self.tok_emb.embedding_layers, self.head.ll_layers):
    ...
    def configure_optimizers(self, train_config):
        ...
        if self.wt:
            ...
            decay.discard('head.ll_layers.0.weight')
            decay.discard('head.ll_layers.1.weight')
    ...
    def forward(self, idx, state, targets=None):
        ...
        if targets is not None:
            ...
            loss = F.cross_entropy(rearrange(logits, '... cl -> (...) cl'), 
...
class MultiGroupLL(nn.Module):
    def __init__(self, input_dim, output_dim, n_group):
        ...
        self.ll_layers = nn.ModuleList([
            ...
            for _ in range(n_group)])
    def forward(self, x):
        x = [mlp(x) for mlp in self.ll_layers]
        x = torch.stack(x, dim=0)
        x = rearrange(x, 'g b t out -> b t g out')
        return x
