...
from pretrain.models.tokenizer import BaseTokenizer

class ActionTokenizer(BaseTokenizer):
    ...
    def __init__(
        ...
        # self.save_hyperparameters(logger=False, ignore="act_vae")
        self.save_hyperparameters(logger=False)
        
        self.use_symlog = use_symlog
        self.act_vae = act_vae
    ...
    def model_step(
        ...
        obs, act, goal = batch
        del obs, goal
        act = self.input_proj(act)
        tar = act.clone().detach()
        act, index, aux_loss, loss_breakdown = self.act_vae.model_step(act)
        rec_loss = self.recon_loss(act, tar)
        ...
        return act, index, total_loss, loss_breakdown
    ...
    def _log_metrics(self, loss_breakdown: Dict[str, MeanMetric], prefix: str) -> None:
        for loss_name, loss_value in loss_breakdown.items():
            ...
            # if loss_name not in self.metrics:
            #     self.metrics[loss_name] = MeanMetric()
            # if loss_name =="train/rec_loss":
    ...
    def training_step(
        ...
        ret_act, _, total_loss, loss_breakdown = self.model_step(batch)
    ...
    def setup(self, stage: str) -> None:
        ...
        if self.hparams.compile and stage == "fit":
            self.act_vae = torch.compile(self.act_vae)
