...
class StateTokenizer(BaseTokenizer):
    ...
    def model_step(
        ...
        obs, act, goal = batch
        del batch
        obs = self.input_proj(obs)
        tar = obs.clone().detach()
        obs, index, aux_loss, loss_breakdown = self.act_vae.model_step(obs)
        rec_loss = self.recon_loss(obs, tar)
        ...
        return obs, index, total_loss, loss_breakdown
